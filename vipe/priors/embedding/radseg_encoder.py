...
from vipe.priors.embedding.base import ImageSemSegEncoder
from segment_anything import sam_model_registry, SamPredictor
from vipe.priors.embedding.sam_utils import sam_refinement
...
class RADSegEncoder(ImageSemSegEncoder):
  ...
  def __init__(self,
               ...
               sam3: bool = False,
               sam_ckpt: str = '/workspace/RayFronts/rayfronts/image_encoders/sam_vit_h_4b8939.pth',
    ...
    if self.sam3:
      self.sam_adaptor = self.model.adaptors["sam3"]
    else:
  ...
  def _single_inference(self, rgb_image):
    suggested = self.model.get_nearest_supported_resolution(*rgb_image.shape[-2:])
    rgb_image = torch.nn.functional.interpolate(
        rgb_image, 
        size=(suggested.height, suggested.width), 
        mode="bilinear", 
        align_corners=False
    )
